...
export const ChartCurrency: React.FC = ({ selectedRows }: any) => {
  ...
  selectedRows && selectedRows.map((selectedRow: SelectedRows) => {
    selectedCurrencies.map((currency: any) => {
      if (currency.id === selectedRow.key) { currenciesNames.push(`${currency.name}-${currency.id}`) }
    })
  });
  const timeData = selectedCurrencies.length && selectedCurrencies[0].dates.map((date: any) => { return new Date(date.id) });
  const currenciesValues: any[] = [];
  selectedRows && selectedRows.map((selectedRow: SelectedRows) => {
    selectedCurrencies.map((currency: any) => { if (currency.id === selectedRow.key) return currenciesValues.push(currency.values) })
  });
  const series: any = [];
  currenciesValues.map((value) => { series.push({ data: Object.values(value).map((v: any) => Number(v[0])) }) });
